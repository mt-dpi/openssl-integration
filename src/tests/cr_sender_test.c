...
{	
  arg_t *arg;
  char *host;
	int i, sock, port, bsize, tbs, tbr, sent, rcvd, offset, nrules, cid;
  uint8_t a, b, c;
  uint8_t *p;
  uint8_t **cert;
  ...
  imsg(DPI_DEBUG_CIRCUIT, "# of Certificates: %d", nrules);
  ...
  for (i=0; i<nrules; i++)
  {
    ...
    while (offset < tbr)
    {
      rcvd = read(sock, cert[i]+offset, bsize-offset);
      offset += rcvd;
    }
    assert(offset == tbr);
    p = cert[i];
    iprint(DPI_DEBUG_CIRCUIT, "Certificate", p, 0, bsize, 16);
    assert(offset == rcvd);
  }
  ...
  dmsg(DPI_DEBUG_INIT, "End of the S's thread: %d", sock);
	close(sock);
  return NULL;
}
