...
#ifdef INTERNAL
unsigned long get_current_clock_time_fg(void)
{
  struct timespec ts;
...
etoken_t *fixedkey_global_token_encryption(dpi_t *dpi, token_t *token)
{
  ...
  int i, rc, hlen, elen, rs, sklen, count, bsize, max_num_of_fetched;
  uint64_t salt;
  uint8_t *skey;
  uint8_t hval[16] = {0, };
  uint8_t eval[16] = {0, };
...
#ifdef INTERNAL
  tstart = start[idx] = get_current_clock_time_fg();
#endif /* INTERNAL */
  context = dpi_get_security_context(dpi);
  table = dpi_get_counter_table(dpi);
#ifdef INTERNAL
  end[idx++] = get_current_clock_time_fg();
#endif /* INTERNAL */

#ifdef INTERNAL
  start[idx] = get_current_clock_time_fg();
#endif /* INTERNAL */
...
#ifdef INTERNAL
  tend = end[idx++] = get_current_clock_time_fg();
#endif /* INTERNAL */
