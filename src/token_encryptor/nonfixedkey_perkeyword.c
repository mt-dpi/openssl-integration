...
#ifdef INTERNAL
unsigned long get_current_clock_time_np(void)
{
  struct timespec ts;
...
etoken_t *nonfixedkey_perkeyword_token_encryption(dpi_t *dpi, token_t *token)
{
  ...
  int rc, etlen, elen, rs, count, bsize;
  uint64_t salt;
  uint8_t etkey[16] = {0, };
  uint8_t enc[16] = {0, };
  uint8_t sbuf[16] = {0, };
...
#ifdef INTERNAL
  tstart = start[idx] = get_current_clock_time_np();
#endif /* INTERNAL */
  context = dpi_get_security_context(dpi);
  table = dpi_get_counter_table(dpi);
#ifdef INTERNAL
  end[idx++] = get_current_clock_time_np();
#endif /* INTERNAL */

#ifdef INTERNAL
  start[idx] = get_current_clock_time_np();
#endif /* INTERNAL */
  ...
  //}
#ifdef INTERNAL
  tend = end[idx++] = get_current_clock_time_np();
#endif /* INTERNAL */
