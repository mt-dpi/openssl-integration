...
#ifdef INTERNAL
unsigned long get_current_clock_time_dfc(void)
{
  struct timespec ts;
...
#ifdef INTERNAL
  tstart = start[i] = get_current_clock_time_dfc();
#endif /* INTERNAL */
	ret = FALSE;
...
#ifdef INTERNAL
  end[i++] = get_current_clock_time_dfc();
#endif /* INTERNAL */

  // Search the encrypted token from the search tree
#ifdef INTERNAL
  start[i] = get_current_clock_time_dfc();
#endif /* INTERNAL */
  count = 0;
...
#ifdef INTERNAL
  tend = get_current_clock_time_dfc();
#endif /* INTERNAL */
