...
void aes128_dec(uint8_t *cipherText, uint8_t *plainText){
...
int main(int argc, char *argv[])
{
  ...
  aes128_enc(plain, ciph);
  iprint(DPI_DEBUG_CIRCUIT, "Plain", plain, 0, 16, 16);
  iprint(DPI_DEBUG_CIRCUIT, "Ciph (mm)", ciph, 0, 16, 16);

  ctx = EVP_CIPHER_CTX_new();
  EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, enc_key, NULL);
  EVP_EncryptUpdate(ctx, ciph, &len, plain, 16);
  clen += len;

  iprint(DPI_DEBUG_CIRCUIT, "Ciph(evp)", ciph, 0, 16, 16);
