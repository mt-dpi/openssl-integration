...
///// Added for MT-DPI /////
static inline void memxor(uint8_t dst[16], const uint8_t *a, const uint8_t *b)
{
...
int mtdpi_token_computation_and_send(SSL *s, const void *buf, size_t len)
{
  printf("mtdpi_token_computation_and_send(): len: %ld\n", len);
  int i, ret, elen, tmp, count;
  unsigned int hlen;
  dpi_t *dpi;
  param_t *param;
  token_t *token;
  etoken_t *etoken;
  uint8_t tokens[SSL3_RT_MAX_PLAIN_LENGTH] = {0, };
  uint8_t *p, *q;
  uint16_t tnum;
  uint64_t salt;
  EVP_CIPHER_CTX *sctx, *ectx;
  EVP_MD_CTX *ctx;
  uint8_t handle[SSL3_RT_MAX_PLAIN_LENGTH][16] = {0, };
  uint8_t result[SSL3_RT_MAX_PLAIN_LENGTH][16] = {0, };
  uint64_t counter[SSL3_RT_MAX_PLAIN_LENGTH] = {1, };
  uint8_t sbuf[16] = {0, };
  uint8_t temp[16] = {0, };
  ...
  get_context_encryption_ctx(context, &ectx);
  get_context_secret_ctx(context, &sctx);

  p = tokens;
  tnum = 0;
  p += 2;
  elen = get_param_rs_value(param);
  ret = tmp = 0;
  ...
  for (i=0; i<len; i++)
  {
    if (len - i >= 8)
      ...
      memcpy(temp, buf+i, len - i);
    EVP_EncryptUpdate(sctx, handle[i], &hlen, temp, 16);
  }

  int j;
  for (i=0; i<len; i++)
  {
    VAR_TO_PTR_8BYTES((salt + counter[i]), sbuf);
    memxor(handle[i], handle[i], sbuf);
    EVP_EncryptUpdate(ectx, result[i], &hlen, handle[i], 16);
    memxor(result[i], result[i], handle[i]);
  }
  ...
  for (i=0; i<len; i++)
  {
    ...
    if (tnum == 3270)
    {
      VAR_TO_PTR_2BYTES(tnum, q);
      tnum = 0;
      ctx = EVP_MD_CTX_create();
      EVP_DigestInit(ctx, EVP_sha256());
      EVP_DigestUpdate(ctx, tokens, 2 + tnum * 5);
      p = tokens + 2 + tnum * 5;
      EVP_DigestFinal(ctx, p, &hlen);
      EVP_MD_CTX_free(ctx);

      dprint(DPI_DEBUG_LIBRARY, "To be sent", tokens, 0, (2 + tnum * 5 + hlen), 16);
      tmp = SSL_write(s, tokens, 2 + 3270 * 5);
      p = tokens;
      ret += tmp;
    }
  }
  ...
  if (tnum > 0)
  {
    ...
    dprint(DPI_DEBUG_LIBRARY, "To be sent", tokens, 0, (2 + tnum * 5 + hlen), 16);
    tmp = SSL_write(s, tokens, 2 + tnum * 5 + hlen);
    p = tokens;
    ret += tmp;
  }
...
int blindbox_token_computation_and_send(SSL *s, const void *buf, size_t len)
{
  printf("blindbox_token_computation_and_send(): len: %ld\n", len);
  int i, ret, elen, tmp, count;
  unsigned int hlen;
  dpi_t *dpi;
  param_t *param;
  ...
  uint8_t handle[SSL3_RT_MAX_PLAIN_LENGTH][16] = {0, };
  uint8_t result[SSL3_RT_MAX_PLAIN_LENGTH][16] = {0, };
  int counter[SSL3_RT_MAX_PLAIN_LENGTH] = {2, };
  uint8_t sbuf[16] = {0, };
  uint8_t temp[16] = {0, };
  security_context_t *context;
  counter_table_t *table;
  ...
  for (i=0; i<len; i++)
  {
    ...
    EVP_EncryptInit_ex(ectx, EVP_aes_128_ecb(), NULL, handle[i], NULL);
    VAR_TO_PTR_8BYTES((salt + counter[i]), sbuf);
    EVP_EncryptUpdate(ectx, result[i], &hlen, sbuf, 16);
  }
...
int mtdpi_token_computation_and_send(SSL *s, const void *buf, size_t len)
{
  ...
  dpi = s->mt_dpi.dpi;
  param = dpi_get_params(dpi);
  dpi_add_message(dpi, (uint8_t *)buf, len);
  p = tokens;
  tnum = 0;
  p += 2;
  elen = get_param_rs_value(param);
  ...
  while (1)
  {
    ...
    if (!token) 
    {
      if (tnum > 0)
      {
        ...
        ctx = EVP_MD_CTX_create();
        EVP_DigestInit(ctx, EVP_sha256());
        EVP_DigestUpdate(ctx, tokens, 2 + tnum * elen);
        EVP_DigestFinal(ctx, p, &hlen);
        EVP_MD_CTX_free(ctx);

        dprint(DPI_DEBUG_LIBRARY, "To be sent", tokens, 0, (2 + tnum * elen + hlen), 16);
        tmp = SSL_write(s, tokens, 2 + tnum * elen + hlen);
        printf("token sent (final): %d\n", tmp);
        p = tokens;
        ret += tmp;
      }
      break;
    ...
    dprint(DPI_DEBUG_LIBRARY, "token", (get_token_value(token)), 0, get_token_length(token), 16);
    etoken = dpi_token_encryption(dpi, token);
    memcpy(p, get_etoken_value(etoken), elen);
    free_etoken(etoken);
    p += elen;
    tnum++;

    if ((tnum + 1) * elen >= SSL3_RT_MAX_PLAIN_LENGTH - SHA256_DIGEST_LENGTH)
    {
      q = tokens;
      VAR_TO_PTR_2BYTES(tnum, q);
  ...
  printf("mtdpi_token_computation_and_send() 1: tnum: %d, elen: %d, hlen: %d\n", tnum, elen, hlen);
      dprint(DPI_DEBUG_LIBRARY, "To be sent", tokens, 0, (2 + tnum * elen + hlen), 16);
      tmp = SSL_write(s, tokens, 2 + tnum * elen + hlen);
  printf("mtdpi_token_computation_and_send() 2\n");
      printf("token sent: %d\n", tmp);
      memset(tokens, 0, SSL3_RT_MAX_PLAIN_LENGTH);
      p = tokens;
      tnum = 0;
      ret += tmp;
    }
...
int mtdpi_token_receive_verification(SSL *s, void *buf, size_t len)
{
  int ret, hlen, elen, tnum;
  uint8_t *p;
  ...
  ctx = EVP_MD_CTX_create();
  EVP_DigestInit(ctx, EVP_sha256());
  EVP_DigestUpdate(ctx, p, tnum * elen);
  EVP_DigestFinal(ctx, hmac, &hlen);
  EVP_MD_CTX_free(ctx);

  if (!strncmp((const char *)hmac,
        (const char *)(buf + 2 + tnum * elen), hlen))
  {
    dmsg(DPI_DEBUG_LIBRARY, "HMAC is verified");
    ret = len - hlen;
  }
  else
  {
    dmsg(DPI_DEBUG_LIBRARY, "HMAC is not verified");
    ret = -1;
  }
...
int ssl3_write(SSL *s, const void *buf, size_t len, size_t *written)
{
    ...
    {
      if (s->mt_dpi.mode == 0)
        ret = mtdpi_token_computation_and_send(s->mt_dpi.mssl, buf, len);
      else if (s->mt_dpi.mode == 1)
        ret = blindbox_token_computation_and_send(s->mt_dpi.mssl, buf, len);
    }
