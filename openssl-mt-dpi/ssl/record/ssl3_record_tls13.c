...
int tls13_enc(SSL *s, SSL3_RECORD *recs, size_t n_recs, int sending)
{
    ...
    if (s->mt_dpi.complete && s->mt_dpi.to_middlebox)
    {
        memmove(rec->data, rec->input, rec->length);
        rec->input = rec->data;
        return 1;
    ...
    if (s->mt_dpi.complete && s->mt_dpi.middlebox)
    {
        memmove(rec->data, rec->input, rec->length);
        rec->input = rec->data;
        return 1;
...
///// Added for MT-DPI /////
int tls_mtdpi_enc(SSL *s, SSL3_RECORD *recs, size_t n_recs, int sending)
{
    EVP_CIPHER_CTX *ctx;
    unsigned char iv[EVP_MAX_IV_LENGTH], recheader[SSL3_RT_HEADER_LENGTH];
    size_t ivlen, taglen, offset, loop, hdrlen;
    ...
    if (s->mt_dpi.complete == 1 
        && (s->mt_dpi.to_middlebox == 1 || s->mt_dpi.middlebox))
    {
