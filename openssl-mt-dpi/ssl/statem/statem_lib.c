...
/*
uint8_t used_secret[16] =
{
  0x83, 0xc1, 0xbb, 0xa1, 0x13, 0x08, 0x77, 0xfa, 
  0x77, 0x0c, 0x50, 0x05, 0x96, 0xb2, 0x58, 0x2d,
};
*/

uint8_t used_encryption_key[16] = 
{
...
///// Added for MT-DPI /////
int open_connection(unsigned char *name, int port);
////////////////////////////
...
int open_connection(unsigned char *name, int port)
{
  ...
  if (!(host = gethostbyname((const char *)name)))
  {
    fd = -1;
    goto out;
  }
  ...
  printf("before connect(): name: %s, port: %d\n", name, port);
...
void *mbconn_run(void *data)
{
  ...
  //method = (SSL_METHOD *)TLS_mtdpi_client_method();
  method = (SSL_METHOD *)TLS_client_method();
  ctx = SSL_CTX_new(method);
  s = (SSL *)data;
  s->mt_dpi.mbsock = open_connection(s->mt_dpi.mbname, MT_DPI_DEFAULT_PORT);
  s->mt_dpi.mssl = SSL_new(ctx);
  s->mt_dpi.mssl->mt_dpi.dpi = s->mt_dpi.dpi;
  SSL_mt_dpi_set_to_middlebox(s->mt_dpi.mssl);
  SSL_mt_dpi_set_master_ssl(s->mt_dpi.mssl, s);
  SSL_set_fd(s->mt_dpi.mssl, s->mt_dpi.mbsock);

  if (s->mt_dpi.mode == 0)
  {
    SSL_enable_mt_dpi(s->mt_dpi.mssl);
  }
  else if (s->mt_dpi.mode == 1)
  {
    SSL_enable_blindbox(s->mt_dpi.mssl);
  }

  dpi_set_ssl_session(s->mt_dpi.mssl->mt_dpi.dpi, s->mt_dpi.mssl);
  slen = 16;
  dmsg(DPI_DEBUG_LIBRARY, "is_to_middlebox: %d\n", SSL_mt_dpi_is_to_middlebox(s->mt_dpi.mssl));
  ...
  if (SSL_connect(s->mt_dpi.mssl) != 1)
  {
    imsg(DPI_DEBUG_LIBRARY, "TLS channel is not established with MB");
  }
  else
  {
    imsg(DPI_DEBUG_LIBRARY, "TLS channel is established with MB");

    /*
    if (s->mt_dpi.mssl->mt_dpi.init)
    {
      imsg(DPI_DEBUG_LIBRARY, "Perform the initialization step");
      RAND_bytes(secret, slen);
      dpi_set_cr_secret_key(s->mt_dpi.dpi, secret, slen);
      dpi_rule_preparation(s->mt_dpi.dpi);
    }
    else
    {
      imsg(DPI_DEBUG_LIBRARY, "Use the hardcoded key");
      iprint(DPI_DEBUG_LIBRARY, "Encryption key", used_encryption_key, 0, slen, slen);
      dpi_set_cr_encryption_key(s->mt_dpi.dpi, used_encryption_key, slen);
      iprint(DPI_DEBUG_LIBRARY, "Secret", used_secret, 0, slen, slen);
      dpi_set_cr_secret_key(s->mt_dpi.dpi, used_secret, slen);
    }
    */

    s->mt_dpi.complete = 1;
    s->mt_dpi.mssl->mt_dpi.complete = 1;
    dpi_set_ssl_session(s->mt_dpi.mssl->mt_dpi.dpi, s->mt_dpi.mssl);
  }
