...
///// Added for MT-DPI /////
EXT_RETURN tls_construct_ctos_mt_dpi(SSL *s, WPACKET *pkt,
                                          unsigned int context, X509 *x,
                                          size_t chainidx)
{
    fstart("s: %p, pkt: %p, context: %u, x: %p, chainidx: %lu", s, pkt, context, x, chainidx);
    SSL *master;
    EVP_MD_CTX *ctx, *actx;
    ...
    uint8_t is_server;
    uint8_t *random;
    uint8_t ticket[SHA_DIGEST_LENGTH];
    uint8_t auth[SHA_DIGEST_LENGTH];

    if (!s->mt_dpi.enabled)
    {
      emsg("MT-DPI is not enabled");
      return EXT_RETURN_NOT_SENT;
    }

    if (s->mt_dpi.to_middlebox)
    {
      master = s->mt_dpi.master;
      if (master->server)
      {
        dmsg(DPI_DEBUG_LIBRARY, "This is the TLS server\n");
        is_server = 1;
      }
      else
      {
        dmsg(DPI_DEBUG_LIBRARY, "This is the TLS client\n");
        is_server = 0;
      }

      // ticket = H(CRandom||SRandom||MTRandom)
      ctx = EVP_MD_CTX_create();
      rc = EVP_DigestInit(ctx, EVP_sha1());
      if (rc != 1) goto err;

      // Client Random
      random = master->s3->client_random;
      dprint(DPI_DEBUG_LIBRARY, "Client Random", random, 0, SSL3_RANDOM_SIZE, 16);
      rc = EVP_DigestUpdate(ctx, random, SSL3_RANDOM_SIZE);
      if (rc != 1) goto err;

      // Server Random
      random = master->s3->server_random;
      dprint(DPI_DEBUG_LIBRARY, "Server Random", random, 0, SSL3_RANDOM_SIZE, 16);
      rc = EVP_DigestUpdate(ctx, random, SSL3_RANDOM_SIZE);
      if (rc != 1) goto err;

      // MT Random
      random = master->mt_dpi.random;
      dprint(DPI_DEBUG_LIBRARY, "MT Random", random, 0, SSL3_RANDOM_SIZE, 16);
      rc = EVP_DigestUpdate(ctx, random, SSL3_RANDOM_SIZE);
      if (rc != 1) goto err;

      // Ticket
      EVP_DigestFinal(ctx, ticket, &tlen);
      if (!ossl_assert(tlen == SHA_DIGEST_LENGTH)) goto err;
      EVP_MD_CTX_free(ctx);
      dprint(DPI_DEBUG_LIBRARY, "Association Key", ticket, 0, tlen, 16);
      memcpy(s->mt_dpi.key, ticket, tlen);
      s->mt_dpi.klen = tlen;

      // auth = H(Random||MTRandom)
      alen = 0;
      actx = EVP_MD_CTX_create();
      rc = EVP_DigestInit(actx, EVP_sha1());
      if (rc != 1) goto err;
      if (master->server)
        random = master->s3->client_random;
      else
        random = master->s3->server_random;
      rc = EVP_DigestUpdate(actx, random, SSL3_RANDOM_SIZE);
      if (rc != 1) goto err;
      random = master->mt_dpi.random;
      rc = EVP_DigestUpdate(actx, random, SSL3_RANDOM_SIZE);
      if (rc != 1) goto err;
      EVP_DigestFinal(actx, auth, &alen);
      if (!ossl_assert(alen == SHA_DIGEST_LENGTH)) goto err;
      EVP_MD_CTX_free(actx);

      dprint(DPI_DEBUG_LIBRARY, "Auth", auth, 0, alen, 16);

      if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_mt_dpi)
              || !WPACKET_start_sub_packet_u16(pkt)
              || !WPACKET_start_sub_packet_u16(pkt)
              || !WPACKET_put_bytes_u8(pkt, is_server)
              || !WPACKET_memcpy(pkt, ticket, SHA_DIGEST_LENGTH)
              || !WPACKET_memcpy(pkt, auth, alen)
              || !WPACKET_close(pkt)
              || !WPACKET_close(pkt))
      {
        ferr();
    ...
    else
    {
      if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_mt_dpi)
              ...
              || !WPACKET_sub_memcpy_u16(pkt, MT_DPI_MB_HOSTNAME, 
                    strlen(MT_DPI_MB_HOSTNAME))
              || !WPACKET_close(pkt)
              || !WPACKET_close(pkt)) {
          ferr();
          return EXT_RETURN_FAIL;
    ...
    ffinish("mbname: %s", MT_DPI_MB_HOSTNAME);
    return EXT_RETURN_SENT;
...
int tls_parse_stoc_mt_dpi(SSL *s, PACKET *pkt, unsigned int context,
                              X509 *x, size_t chainidx)
{
    fstart("s: %p, pkt: %p, context: %u, x: %p, chainidx: %lu", s, pkt, context, x, chainidx);

    int rc;
    unsigned int mtype, nlen, alen, ralen, init;
    const unsigned char *mbname, *random, *rauth;
    PACKET mtdpi;
    pthread_t *mbconn;
    pthread_attr_t *attr;
    ...
    if (!s->mt_dpi.to_middlebox)
    {
      if (!PACKET_as_length_prefixed_2(pkt, &mtdpi)
          || PACKET_remaining(&mtdpi) == 0) {
          ferr();
          return 0;
      }

      if (!PACKET_get_1(&mtdpi, &mtype)
          || mtype != TLSEXT_NAMETYPE_host_name
          || !PACKET_get_net_2(&mtdpi, &nlen)
          || !PACKET_get_bytes(&mtdpi, &mbname, nlen)
          || !PACKET_get_bytes(&mtdpi, &random, SSL3_RANDOM_SIZE)) {
          ferr();
          return 0;
      }
    
      OPENSSL_free(s->mt_dpi.mbname);
      s->mt_dpi.mbname = NULL;
      s->mt_dpi.mbname = (unsigned char *)calloc(1, nlen+1);
      memcpy(s->mt_dpi.mbname, mbname, nlen);
      memcpy(s->mt_dpi.random, random, SSL3_RANDOM_SIZE);

      imsg(DPI_DEBUG_LIBRARY, "MB name: %s", s->mt_dpi.mbname);
      random = s->mt_dpi.random;
      iprint(DPI_DEBUG_LIBRARY, "MT-DPI Random", random, 0, SSL3_RANDOM_SIZE, 16);

      s->mt_dpi.conf = init_conf_module();
      set_conf_exp_use_hardcoded_keys(s->mt_dpi.conf);
      if (s->mt_dpi.mode == 0)
      {
        imsg(DPI_DEBUG_LIBRARY, "MT-DPI is set");
        set_conf_module_rule_preparation_idx(s->mt_dpi.conf, 
            FIXEDKEY_PERKEYWORD_RULE_PREPARER_IDX);
        set_conf_module_get_next_token_idx(s->mt_dpi.conf, WINDOW_BASED_TOKENIZER_IDX);
        set_conf_module_token_encryption_idx(s->mt_dpi.conf, 
            FIXEDKEY_PERKEYWORD_TOKEN_ENCRYPTOR_IDX);
        set_conf_module_token_detection_idx(s->mt_dpi.conf, 
            FIXEDKEY_PERKEYWORD_TOKEN_DETECTOR_IDX);
        set_conf_module_tree_update_idx(s->mt_dpi.conf, FIXEDKEY_PERKEYWORD_TREE_UPDATER_IDX);
        set_conf_param_window_size(s->mt_dpi.conf, 8);
        set_conf_param_token_size(s->mt_dpi.conf, 5);
      }
      else if (s->mt_dpi.mode == 1)
      {
        imsg(DPI_DEBUG_LIBRARY, "BlindBox is set");
        set_conf_module_rule_preparation_idx(s->mt_dpi.conf, 
            NONFIXEDKEY_PERKEYWORD_RULE_PREPARER_IDX);
        set_conf_module_get_next_token_idx(s->mt_dpi.conf, WINDOW_BASED_TOKENIZER_IDX);
        set_conf_module_token_encryption_idx(s->mt_dpi.conf, 
            NONFIXEDKEY_PERKEYWORD_TOKEN_ENCRYPTOR_IDX);
        set_conf_module_token_detection_idx(s->mt_dpi.conf, 
            NONFIXEDKEY_PERKEYWORD_TOKEN_DETECTOR_IDX);
        set_conf_module_tree_update_idx(s->mt_dpi.conf, NONFIXEDKEY_PERKEYWORD_TREE_UPDATER_IDX);
        set_conf_param_window_size(s->mt_dpi.conf, 8);
        set_conf_param_token_size(s->mt_dpi.conf, 5);
      }

      s->mt_dpi.dpi = init_dpi_context(DPI_ROLE_CLIENT, s->mt_dpi.conf);
      dpi_set_ssl_session(s->mt_dpi.dpi, s);

      attr = (pthread_attr_t *)calloc(1, sizeof(pthread_attr_t));
      pthread_attr_init(attr);
      pthread_attr_setdetachstate(attr, PTHREAD_CREATE_JOINABLE);
      mbconn = (pthread_t *)calloc(1, sizeof(pthread_t));
      pthread_create(mbconn, attr, mbconn_run, s);
    }
    else
    {
      if (!PACKET_as_length_prefixed_2(pkt, &mtdpi)
      ...
      ralen = SHA_DIGEST_LENGTH;
      if (!PACKET_get_bytes(&mtdpi, &rauth, ralen)
          || !PACKET_get_1(&mtdpi, &init)) {
          ferr();
          return 0;
      }

      dprint(DPI_DEBUG_LIBRARY, "Received Auth", rauth, 0, ralen, 16);
      ...
      dprint(DPI_DEBUG_LIBRARY, "Generated Auth", auth, 0, alen, 16);

      // Verify auth ?= rauth
      if (strncmp((const char *)auth, (const char *)rauth, alen))
      {
        dmsg(DPI_DEBUG_LIBRARY, "verification failed");
        ferr();
        return 0;
      }
      dmsg(DPI_DEBUG_LIBRARY, "verification succeed");

      s->mt_dpi.init = init;
      s->mt_dpi.master->mt_dpi.init = init;
    }

    s->ext.use_mt_dpi = 1;
    ffinish("s->ext.use_mt_dpi: %d", s->ext.use_mt_dpi);
