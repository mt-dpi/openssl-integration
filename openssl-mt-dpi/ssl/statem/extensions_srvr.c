...
///// Added for MT-DPI /////
int tls_parse_ctos_mt_dpi(SSL *s, PACKET *pkt, unsigned int context,
                                       X509 *x, size_t chainidx)
{
    ...
    if (!s->mt_dpi.middlebox)
    {
      ...
      dmsg(DPI_DEBUG_LIBRARY, "PACKET_remaining(&mtdpi): %ld", PACKET_remaining(&mtdpi));

      if (!PACKET_get_1(&mtdpi, &mtype)
          || mtype != TLSEXT_NAMETYPE_host_name
          || !PACKET_as_length_prefixed_2(&mtdpi, &mbname)) {
          SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_F_TLS_PARSE_CTOS_SERVER_NAME,
                   SSL_R_BAD_EXTENSION);
          return 0;
      }

      if (PACKET_remaining(&mbname) > TLSEXT_MAXLEN_host_name) {
      ...
      if (PACKET_contains_zero_byte(&mbname)) {
          SSLfatal(s, SSL_AD_UNRECOGNIZED_NAME,
                    SSL_F_TLS_PARSE_CTOS_SERVER_NAME,
                    SSL_R_BAD_EXTENSION);
          return 0;
      ...
      if (!PACKET_strndup(&mbname, &s->mt_dpi.mbname)) {
      ...
      if (!s->mt_dpi.server)
      {
        dmsg(DPI_DEBUG_LIBRARY, "MB name: %s", s->mt_dpi.mbname);
        s->mt_dpi.server = 1;
        RAND_bytes(s->mt_dpi.random, SSL3_RANDOM_SIZE);
        random = s->mt_dpi.random;

        dprint(DPI_DEBUG_LIBRARY, "MT-DPI Random", random, 0, SSL3_RANDOM_SIZE, 16);

        random = s->s3->client_random;
        dprint(DPI_DEBUG_LIBRARY, "Client Random (extensions_srvr.c)", random, 0, SSL3_RANDOM_SIZE, 16);
        random = s->s3->server_random;
        dprint(DPI_DEBUG_LIBRARY, "Server Random (extensions_srvr.c)", random, 0, SSL3_RANDOM_SIZE, 16);
    ...
    else
    {
      ...
      if (mtype == 1)
      {
        dmsg(DPI_DEBUG_LIBRARY, "The TLS server is connected");
        SSL_mt_dpi_set_server(s);
      }
      else
      {
        dmsg(DPI_DEBUG_LIBRARY, "The TLS client is connected");
      }

      if (!PACKET_get_bytes(&mtdpi, &data, SHA_DIGEST_LENGTH))
      {
        ferr();
        return 0;
      }

      dprint(DPI_DEBUG_LIBRARY, "Association Key", data, 0, SHA_DIGEST_LENGTH, 16);
      s->mt_dpi.klen = SHA_DIGEST_LENGTH;
      memcpy(s->mt_dpi.key, data, SHA_DIGEST_LENGTH);
      ...
      dprint(DPI_DEBUG_LIBRARY, "Auth", data, 0, SHA_DIGEST_LENGTH, 16);
      s->mt_dpi.alen = SHA_DIGEST_LENGTH;
      memcpy(s->mt_dpi.auth, data, SHA_DIGEST_LENGTH);

      if (mtype == 1)
        update_channel(s->mt_dpi.table, s->mt_dpi.key, s->mt_dpi.klen, s, MT_DPI_TLS_SERVER);
      else
        update_channel(s->mt_dpi.table, s->mt_dpi.key, s->mt_dpi.klen, s, MT_DPI_TLS_CLIENT);
      s->mt_dpi.dpi = get_associated_dpi_context(s->mt_dpi.table, 
          s->mt_dpi.key, s->mt_dpi.klen);
    }
...
///// Added for MT-DPI /////
EXT_RETURN tls_construct_stoc_mt_dpi(SSL *s, WPACKET *pkt,
                                          unsigned int context, X509 *x,
                                          size_t chainidx)
{
    fstart("s: %p, pkt: %p, context: %u, x: %p, chainidx: %lu", s, pkt, context, x, chainidx);
    ...
    if (SSL_mt_dpi_is_middlebox(s))
    {
      peer = NULL;
      dmsg(DPI_DEBUG_LIBRARY, "before get_peer_ssl(): TLS server? %d", SSL_mt_dpi_is_server(s));
      while (!peer)
        peer = get_peer_ssl(s->mt_dpi.table, s->mt_dpi.key, s->mt_dpi.klen, s);
      dmsg(DPI_DEBUG_LIBRARY, "after get_peer_ssl(): peer: %p, TLS server? %d", peer, SSL_mt_dpi_is_server(peer));
      auth = SSL_mt_dpi_get_auth_value(peer, &alen);
      
      dprint(DPI_DEBUG_LIBRARY, "Auth", auth, 0, alen, 16);
      assert(auth != NULL);
      assert(alen > 0);

      dmsg(DPI_DEBUG_LIBRARY, "Initialization enabled: %d\n", s->mt_dpi.init);
      if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_mt_dpi)
              || !WPACKET_start_sub_packet_u16(pkt)
              || !WPACKET_start_sub_packet_u16(pkt)
              || !WPACKET_memcpy(pkt, auth, alen)
              || !WPACKET_put_bytes_u8(pkt, s->mt_dpi.init)
              || !WPACKET_close(pkt)
              || !WPACKET_close(pkt)) {
          ferr();
          return EXT_RETURN_FAIL;
    ...
    else
    {
      if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_mt_dpi)
              ...
              || !WPACKET_sub_memcpy_u16(pkt, MT_DPI_MB_HOSTNAME, strlen(MT_DPI_MB_HOSTNAME))
              || !WPACKET_memcpy(pkt, s->mt_dpi.random, SSL3_RANDOM_SIZE)
              || !WPACKET_close(pkt)
              || !WPACKET_close(pkt)) {
